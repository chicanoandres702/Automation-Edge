...
import { Dialog, DialogContent, DialogTitle, DialogDescription, ScrollArea, Button, Slider, Badge, Switch, Label } from "@/ui";
import {
   Settings,
   Globe,
   Trash2,
...
// Slider moved to shared ui barrel
import { useState, useEffect, useRef, useId, useCallback } from "react";
import { Copy, Eye, EyeOff } from "lucide-react";
// Badge moved to shared ui barrel
import { cn } from "@/lib/utils";
// Switch moved to shared ui barrel
      ...
      try {
         ...
         if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
            chrome.storage.local.get(['ai_api_key', 'actionDelayMs'], (res: any) => {
               ...
               if (!storedKey && envKey) {
                  setTimeout(() => setAiKey(envKey), 0);
                  try { chrome.storage.local.set({ ai_api_key: envKey, actionDelayMs: storedDelay }); } catch { }
               } else {
                  setTimeout(() => setAiKey(storedKey), 0);
               }
               setTimeout(() => setActionDelay(storedDelay), 0);
            });
         } else {
            ...
            if (!storedKey && envKey) {
               setTimeout(() => setAiKey(envKey), 0);
               try { localStorage.setItem('ai_api_key', envKey); localStorage.setItem('actionDelayMs', String(storedDelay)); } catch { }
            } else {
               setTimeout(() => setAiKey(storedKey), 0);
            }
            setTimeout(() => setActionDelay(storedDelay), 0);
                     ...
                     {activeTab === 'integration' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-2 duration-500">
                           ...
                           <div className="p-5 rounded-2xl bg-white/[0.03] border border-white/10 space-y-4">
                              <div className="space-y-2">
                                 <Label htmlFor="ai-api-key" className="text-[9px] font-black uppercase tracking-widest">AI API Key</Label>
                                 <div className="flex items-center gap-2">
                                    <input
                                       id="ai-api-key"
                                    ...
                                    />
                                    <Button variant="ghost" size="icon" onClick={() => { try { navigator.clipboard.writeText(aiKey || ''); } catch { } }}>
                              ...
                              <div className="space-y-2">
                                 <Label htmlFor="action-delay-ms" className="text-[9px] font-black uppercase tracking-widest">Action Delay (ms)</Label>
                                 <div className="flex items-center gap-3">
                                    <input
                                       id="action-delay-ms"
